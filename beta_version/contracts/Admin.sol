...
contract Admin{
    ...
    event signerApproved(address who);
    event upgradeApproved(address _upgrade);
	//event signerRejected(address who);
    //event upgradeRejected(address _upgrade);
     ...
     */
    constructor(address[] memory _signers, uint256 _threshold, address payable _proxy) payable {
     ...
     */
    function approveUpgrade() public payable {
        //require(isSigner[msg.sender], "You don't have permission to vote");
    ...
    /*function rejectUpgrade() public {
        /*require(isSigner[msg.sender], "You don't have permission to approve an upgrade");
        require(upgradeQueded, "There isn't an upgrade waiting to be approved");
        require(!voted[msg.sender], "You already approved the upgrade");
        voted[msg.sender] = true;
        emit signerRejected(msg.sender);
        rejects++;
        if(rejects >= (signers.length - threshold)) {
            emit upgradeRejected(upgrade);
            restarVoting();
        }
    }*/
